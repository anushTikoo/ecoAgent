...
async def first_question(session_id: str) -> Dict[str, Any]:
    ...
    row = await connection.fetchrow("""
        SELECT company_profile, current_sector
        FROM sessions
        WHERE session_id = $1
    """, session_id)
    ...
    await connection.execute("""
        ...
        WHERE session_id = $2
    """,
        llm_json.get("next_sector") or row["current_sector"],
        session_id
    )
...
async def next_question(req_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # ---------- STORE Q & A ----------
    await connection.execute("""
        INSERT INTO qa_messages (session_id, sector, question_text, answer_text)
        VALUES ($1, $2, $3, $4)
    """, session_id, sector, question, answer)
    ...
    await connection.execute("""
        INSERT INTO vector_memory (session_id, content, sector, embedding)
        VALUES ($1, $2, $3, $4)
    """, session_id, content, sector, entry_embedding)

    # ---------- FETCH SESSION DATA ----------
    session_row = await connection.fetchrow("""
        SELECT company_profile, summary_text, current_sector, missing_fields
        FROM sessions WHERE session_id = $1
    """, session_id)

    # ---------- Q/A IN CURRENT SECTOR ----------
    qa_sector_rows = await connection.fetch("""
        SELECT question_text, answer_text
        FROM qa_messages
        WHERE session_id = $1 AND sector = $2
        ORDER BY created_at ASC
    """, session_id, sector)

    qa_in_sector = [
        {"question": r["question_text"], "answer": r["answer_text"]}
        for r in qa_sector_rows
    ]
    ...
    data = {
        ...
        "current_sector": session_row["current_sector"],
    ...
    for sf in extracted_fields:
        await connection.execute("""
            INSERT INTO structured_fields (
                session_id, sector, entity_id, field_name,
    ...
    await connection.execute("""
        ...
        WHERE session_id = $4
    """,
        llm_json.get("next_sector") or session_row["current_sector"],
        json.dumps(llm_json.get("updated_missing_field") or []),
        llm_json.get("sector_complete", False),
        session_id
    )
